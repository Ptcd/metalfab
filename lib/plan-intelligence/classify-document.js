...
const FILENAME_PATTERNS = [
  ...
  { kind: 'specification', re: /\bspecs?\b|specification|project[\s_-]?manual/i },
  { kind: 'drawing',     re: /\bplan[\s_-]?sheet|drawing|elevation|section|detail|sheet/i },
...
const SHEET_PREFIXES = ['A', 'S', 'M', 'P', 'E', 'C', 'L', 'T', 'FP', 'FA'];

function joinPageText(page, limit = 4000) {
  const items = page.items.slice(0, 600);
  let s = items.map((it) => it.str).join(' ');
...
function extractSheetIdentity(page) {
  ...
  let sheet_no = null;
  // Look for sheet ids in known prefix space, e.g. S101, P501, A-201, M2.1
  const re = new RegExp(
    `\\b(${SHEET_PREFIXES.join('|')})[- ]?(\\d{1,3}(?:\\.\\d{1,2})?)\\b`,
    'g'
  );
  const matches = [...tbText.matchAll(re)];
  if (matches.length) {
    // Prefer the shortest, most "title-block-like" match (S101 over S101.5)
