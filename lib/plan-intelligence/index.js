 ...
 *
 * Walks ALL docs, not just `kind === 'specification'`. Combined
 * permit sets (drawings + spec book in one PDF, e.g. Nestle's
 * `4_Nestle_SCH-FAC_BID-PERMIT_SET.pdf`) get classified as drawings
 * by filename, but the spec content is still in there — we'd miss it
 * if we filtered to spec-only docs.
 */
function extractTcbSections(allDocs) {
  const found = new Map();
  for (const d of allDocs) {
    if (!d._pages) continue;
    // Pre-build regex pairs once per doc to avoid recompiling per page.
    // Anchor at word boundary so cross-references like "see 05 50 00"
    // don't match — only "SECTION 05 50 00" or page-number "05 50 00 - 1".
...
/**
 * Extract EVERY CSI section header found anywhere in the package, not
 * just the hardcoded TCB-relevant 20. The coverage manifest stage
 * consumes this to enumerate the full scope envelope (so excluded
 * sections like Div 09 paint can be carried as explicit exclusions in
 * the takeoff).
 *
 * Walks all docs (combined permit sets matter — see extractTcbSections
 * note). Captures the section title from the same line when present.
 *
 * Returns: [{ code, title, source_filename, first_page }, ...]
 */
function extractAllSpecSections(allDocs) {
  // Match "SECTION 05 50 00 METAL FABRICATIONS" or "DOCUMENT 05 50 00 - METAL FABRICATIONS".
  // Captures: 1=code, 2=optional title (uppercase letters/digits/spaces/punct, ≤80 chars).
  const HEADER_RE = /\b(?:SECTION|DOCUMENT)[\s:.\-]+(\d{2}[\s\-.]\d{2}[\s\-.]\d{2})(?:[\s:.\-]+([A-Z][A-Z0-9 ,&\-/()]{2,80}))?/gi;

  const found = new Map();   // normalized code -> first-occurrence record
  for (const d of allDocs) {
    if (!d._pages) continue;
    for (const p of d._pages) {
      const text = p.items.map((i) => i.str).join(' ');
      ...
      while ((m = HEADER_RE.exec(text)) !== null) {
        ...
        // Trim trailing noise like "PART 1" or "1.01" that often immediately
        // follows the title in spec headers.
...
async function processPackage(documents) {
  ...
  // Walk ALL docs — combined permit sets (drawings + spec book in one
  // PDF) get filename-classified as drawings but contain spec content.
  const tcbSections = extractTcbSections(docs);
  // Full CSI section index — every header found anywhere, not just
  // the hardcoded TCB-relevant 20. Consumed by the coverage manifest
  // stage to build a complete in-scope/excluded envelope.
  const specSectionIndex = extractAllSpecSections(docs);
  ...
  const summary = {
    ...
    spec_section_index: specSectionIndex,
