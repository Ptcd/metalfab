...
// xlsx via SheetJS (native), pdf via lib/plan-intelligence/extract-text.
...
/**
 * Extract CSI codes from an xlsx bid form. Native JS via SheetJS — no
 * Python subprocess. Removes the cross-platform Python+pandas dependency
 * the previous implementation needed.
 *
 * Returns array of { code, description, sheet, row }.
 */
function parseXlsxBidForm(filepath) {
  ...
  try {
    ...
    console.error('parseXlsxBidForm: xlsx package not installed. Run `npm install xlsx`.');
    return [];
  }
  let workbook;
  try {
    ...
    console.error('parseXlsxBidForm: cannot read', filepath, '-', e.message?.slice(0, 200));
    return [];
  }
  const codes = [];
  for (const sheetName of workbook.SheetNames) {
    const ws = workbook.Sheets[sheetName];
    // Convert to array-of-arrays so we preserve row/column structure
