...
// Match sheet numbers like A000, A101A, G201, FP-101, S101.1. The
// strict form requires 3+ digits OR an alpha suffix to avoid matching
// page-reference shorthand like "p21" or "S5" — those are 1-2 digit
// page numbers in informal evidence text. Sheet schedules use 3-4
// digit numbering (A101, G203) or 3-digit + alpha (A101A).
const SHEET_NUMBER_RE = /\b(?:FP|FA|[GASMEPCLTID])\d{1,4}(?:[A-Z]\d?)?(?:\.\d+)?\b/;
const STRICT_SHEET_NUMBER_RE = /\b(?:FP|FA|[GASMEPCLTID])(?:\d{3,4}(?:[A-Z]\d?)?(?:\.\d+)?)\b/;
 ...
 *   "DET. 5 ON A001"
 */
// Reference patterns. Each MUST require either 3+ digit sheet numbers,
// or an alpha suffix, or a decimal point — to avoid matching informal
// page references like "p21" or "p. 27" which use 1-2 digit numbers.
// "SHEET A12" is unusual; "SHEET A102" is normal.
const REF_PATTERNS = [
  // "Detail 3/A060" or "Det. 5/A001" — case-insensitive on DETAIL/DET keyword,
  // case-sensitive on the discipline letter (so "p21" lowercase stays out).
  /\b(?:DETAIL|DET\.?)\s+\d+\s*\/\s*((?:FP|FA|[GASMEPCLTID])(?:\d{3,4}(?:[A-Z]\d?)?(?:\.\d+)?))\b/g,
  // "5/A001" naked — capture group must be a strict sheet number
  /\b\d+\s*\/\s*((?:FP|FA|[GASMEPCLTID])(?:\d{3,4}(?:[A-Z]\d?)?(?:\.\d+)?))\b/g,
  // "SHEET A301"
  /\bSHEET\s+((?:FP|FA|[GASMEPCLTID])(?:\d{3,4}(?:[A-Z]\d?)?(?:\.\d+)?))\b/gi,
  // "ON A301" or "on S101" — case-insensitive on keyword. The strict
  // sheet-number alternation rules out "p21" (only 2 digits, no alpha
  // suffix, no decimal) so case-insensitive is safe here.
  /\bON\s+((?:FP|FA|[GASMEPCLTID])(?:\d{3,4}(?:[A-Z]\d?)?(?:\.\d+)?))\b/gi,
];
