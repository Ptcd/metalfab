...
function summarizeDoorSchedule(scheduleOrList, options = {}) {
  ...
  // Confidence flag: row clustering can split a single logical row across
  // two y-clusters when cells wrap to a second line, OR merge two rows
  // when row spacing is tight. We can't fix this from data alone, but
  // we can flag suspicious counts so the takeoff agent vision-verifies.
  // Heuristic: if total parsed rows is < 6 OR > 35, or if tcb_total is
  // < 3 on a final-CD package (which usually has 8+ HM doors), flag.
  const suspicious =
    allRows.length < 6 || allRows.length > 35 ||
    (counts.tcb_total < 3 && counts.total >= 5);
  if (suspicious) {
    counts.parse_confidence = 'low';
    counts.parse_concerns = [];
    if (allRows.length < 6) counts.parse_concerns.push(`only ${allRows.length} rows parsed — door schedules typically 8-30 rows`);
    if (allRows.length > 35) counts.parse_concerns.push(`${allRows.length} rows parsed — likely include non-schedule items captured by row clusterer`);
    if (counts.tcb_total < 3 && counts.total >= 5) counts.parse_concerns.push(`only ${counts.tcb_total} HM frames out of ${counts.total} rows — frame-material column may have been mis-bucketed`);
  } else {
    counts.parse_confidence = 'high';
  }
  return counts;
}
