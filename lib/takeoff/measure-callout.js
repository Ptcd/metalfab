...
// Accept feet-inches dimensions in their many architect-written forms:
//   "12'-0\""              standard
//   "12' - 0\""             with spaces
//   "12'-0\" V.I.F."        with V.I.F. suffix
//   "9'-8\" +/-"            with +/- tolerance suffix
//   "5'-0 1/4\""            with mixed-fraction inches
//   "10'"                   bare feet (no inches)
const DIM_RE = /^(\d+)'(?:\s*-?\s*(\d+)(?:\s*(\d+)\/(\d+))?\s*"?)?\s*(?:\+\/-|V\.I\.F\.)?$/;
...
function parseDim(s) {
  ...
  let frac = 0;
  if (m[3] && m[4]) {
    const a = parseInt(m[3], 10), b = parseInt(m[4], 10);
    if (b > 0) frac = a / b;
  }
