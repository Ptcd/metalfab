...
/* ---------------- Manifest coverage (run-level, BLOCKING) ---------------- */

/**
 * Reconcile the takeoff against the coverage manifest. This is the
 * completeness invariant — every `included` manifest entry must end up
 * covered by takeoff lines OR explicitly listed in
 * manifest_reconciliation.intentionally_excluded with a reason.
 *
 * Severity 'error' here is BLOCKING — takeoff-commit refuses to
 * insert the run if any manifest_coverage_missing error fires.
 *
 * @param {Object|null} manifest         coverage_manifest from context.json
 * @param {Object}      takeoff          parsed takeoff.json (full doc, not just lines)
 * @param {Object[]}    lines            enriched takeoff lines
 */
function validateManifestCoverage(manifest, takeoff, lines) {
  // No manifest = backwards-compat warning, not blocking. The user is
  // running an opp from before the coverage stage existed.
  if (!manifest) {
    return [{
      severity: 'warning',
      category: 'no_coverage_manifest',
      finding: 'No coverage_manifest in context.json. Run scripts/coverage.js to enable completeness checking. Without a manifest, "no findings" only means "nothing tripped a heuristic" — not "every in-scope item was accounted for".',
      recommendation: 'Run: node scripts/coverage.js --opp=<opp_id> before takeoff-prepare.',
    }];
  }

  const findings = [];
  const reconciliation = takeoff?.manifest_reconciliation || {};
  const covered = reconciliation.covered || [];
  const intentionallyExcluded = reconciliation.intentionally_excluded || [];
  const visionReadsCompleted = new Set(reconciliation.vision_reads_completed || []);

  // Index covered + excluded by (kind, ref) for fast lookup
  const coveredKey = new Set(covered.map((c) => `${c.kind}::${c.ref}`));
  const excludedKey = new Map(intentionallyExcluded.map((e) => [`${e.kind}::${e.ref}`, e]));

  // Also: a takeoff line that cites a section in `source_section` counts
  // as covering that spec section even if `manifest_reconciliation.covered`
  // doesn't list it explicitly. This makes the invariant lenient on the
  // reconciliation block as long as the actual lines provide coverage.
  const lineSectionCoverage = new Set(
    (lines || [])
      .map((l) => String(l.source_section || '').trim())
      .filter(Boolean)
  );

  // 1) Every included spec section
  for (const s of (manifest.spec_sections || [])) {
    if (s.tag !== 'included') continue;
    const key = `spec_section::${s.code}`;
    if (coveredKey.has(key) || lineSectionCoverage.has(s.code)) continue;
    if (excludedKey.has(key)) {
      const exc = excludedKey.get(key);
      if (!exc.reason || exc.reason.length < 12) {
        findings.push({
          severity: 'error',
          category: 'manifest_coverage_missing',
          finding: `Spec section ${s.code} (${s.title || 'untitled'}) is intentionally_excluded but the reason is too short / missing. Provide a reason that overrides the TCB scope policy (typically a Q&A clarification or owner directive).`,
          recommendation: 'Either cover the section with a takeoff line or supply a substantive reason for excluding it.',
        });
      }
      continue;
    }
    findings.push({
      severity: 'error',
      category: 'manifest_coverage_missing',
      finding: `Spec section ${s.code} (${s.title || 'untitled'}) is tagged 'included' in the coverage manifest (source: ${s.source_filename || 'spec book'} p${s.first_page ?? '?'}) but no takeoff line covers it and it is not in intentionally_excluded. Expected categories: ${(s.expected_categories || []).join(', ') || '(none — review section)'}.`,
      recommendation: `Add a takeoff line citing source_section='${s.code}' OR add an intentionally_excluded entry with a reason.`,
    });
  }

  // 2) Every needs_vision plan sheet must be in vision_reads_completed
  for (const p of (manifest.plan_sheets || [])) {
    if (!p.needs_vision) continue;
    if (p.tag === 'excluded' || p.tag === 'n_a') continue;
    const ref = p.sheet_no || `p${p.page_number}`;
    if (visionReadsCompleted.has(ref)) continue;
    findings.push({
      severity: 'error',
      category: 'manifest_coverage_missing',
      finding: `Plan sheet ${ref} (${p.sheet_title || p.discipline || 'untitled'}, ${p.source_filename}) was flagged needs_vision=true (${p.vision_reason || 'thin text extraction'}) but does not appear in manifest_reconciliation.vision_reads_completed. Sheets like this are exactly how scope gets missed (G900-G910 class).`,
      recommendation: `Open ${ref} with the Read tool, surface any TCB-scope items as takeoff lines, then add "${ref}" to manifest_reconciliation.vision_reads_completed.`,
    });
  }

  // 3) Every included schedule
  for (const sch of (manifest.schedules || [])) {
    if (sch.tag !== 'included') continue;
    const key = `schedule::${sch.kind}`;
    if (coveredKey.has(key)) continue;
    if (excludedKey.has(key)) continue;
    findings.push({
      severity: 'error',
      category: 'manifest_coverage_missing',
      finding: `Schedule '${sch.kind}' (${sch.source_filename} p${sch.page_number}, ${sch.row_count || '?'} rows) is tagged 'included' in the coverage manifest but no takeoff line cites it. Schedules are the highest-confidence scope source — leaving one uncovered usually means missing items.`,
      recommendation: `Read the schedule, add takeoff lines for in-scope rows, and add a manifest_reconciliation.covered entry with kind='schedule', ref='${sch.kind}'.`,
    });
  }
 ...
 * @param {Object|null} ctx.coverageManifest  coverage_manifest (if present)
 * @param {Object|null} ctx.takeoffDoc        full takeoff.json (for manifest_reconciliation block)
 */
function validateLines(lines, ctx) {
  ...
  // Coverage manifest reconciliation — BLOCKING when errors fire.
  // ctx.takeoffDoc is the full parsed takeoff.json so we can read
  // manifest_reconciliation. ctx.coverageManifest is null when the
  // coverage stage hasn't been run for this opp.
  if (ctx.coverageManifest !== undefined || ctx.takeoffDoc) {
    allFindings.push(...validateManifestCoverage(ctx.coverageManifest || null, ctx.takeoffDoc || {}, updated));
  }
