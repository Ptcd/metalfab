...
function validateSpecPagesPopulated(ctx) {
  ...
  // body_text_length suggest only title-block boilerplate.
  ...
  for (const s of ctx.sheets) {
    ...
    candidates.push({ sheet_no: s.sheet_no, sheet_title: title, body_text_length: s.body_text_length, page: s.page_number, template_cluster_size: bucketSize });
  }
  if (candidates.length === 0) return [];

  // CRITICAL: text-extraction returning low body length on a SPECIFICATIONS
  // sheet does NOT mean the sheet is blank. Many CAD systems publish spec
  // sheets with ALL TEXT CONVERTED TO VECTOR OUTLINES (paths, not text
  // objects). pdfjs returns nothing extractable, but the spec content is
  // visually present and complete. We discovered this on Nestle: G900-G910
  // looked "blank" via text extraction but contain the full project manual
  // when read visually.
  //
  // So this validator no longer fires `spec_pages_blank` as ERROR. Instead
  // it fires `spec_pages_text_extraction_failed` as a WARNING that says
  // "look at the rendered PNG for these sheets — text extraction missed
  // them and they may contain real content." If the agent then reads the
  // PNGs and finds real content, they cite it from the visual; if the
  // PNGs are genuinely blank, the agent escalates to the GC.
  const renderableNote = ctx.rendersDir
    ? ` Renders are available in ${require('path').basename(ctx.rendersDir)}/.`
    : ` Auto-render didn't include these pages — open them manually with the Read tool.`;
  const list = candidates.map((e) => `${e.sheet_no} (p${e.page}, ${e.body_text_length}c body text${e.template_cluster_size >= 3 ? `, identical to ${e.template_cluster_size - 1} sibling sheets` : ''})`).join('; ');
  const titleType = candidates[0].sheet_title.toUpperCase().includes('SPEC') ? 'SPECIFICATIONS'
                  : candidates[0].sheet_title.toUpperCase().includes('NOTE') ? 'GENERAL NOTES'
                  : 'SCHEDULE';
  findings.push({
    severity: 'warning',
    category: 'spec_pages_text_extraction_failed',
    finding: `${candidates.length} sheet(s) titled "${titleType}" returned little/no body text from PDF text extraction: ${list}. This is OFTEN a CAD-rendered spec sheet where text was converted to vector outlines — the spec content IS present visually but unreadable to text-only validators. DO NOT assume the sheets are blank.`,
    recommendation: `Open the rendered PNGs of these pages with the Read tool and visually inspect for actual spec text. If real spec content is present, cite the verbatim spec language in source_evidence (now possible since text doesn't need to come from extraction). If the pages truly are blank when viewed, THEN escalate as RFI to GC.${renderableNote}`,
    related_takeoff_line: null,
  });
  return findings;
}
